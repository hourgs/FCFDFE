...
{
    public partial class AccountModify : Page
    {
        ...
        private void dataImport()
        {
            ...
            //帳號資料
            var account = GME.ACCOUNTs.Where(table => table.USER_ID.Equals(strUSER_ID)).FirstOrDefault();
            if (account != null)
            {
                txtPWD.Text = account.PWD;
                txtPWDConfirm.Text = account.PWD;
                txtUSER_NAME.Text = account.USER_NAME;
                string strDEPT_SN = account.DEPT_SN;
                TBMDEPT dept = GME.TBMDEPTs.Where(table => table.OVC_DEPT_CDE.Equals(strDEPT_SN)).FirstOrDefault();
                txtOVC_DEPT_CDE.Text = strDEPT_SN;
                txtOVC_ONNAME.Text = dept != null ? dept.OVC_ONNAME : "";
                txtIUSER_PHONE.Text = account.IUSER_PHONE;
                txtEMAIL_ACCOUNT.Text = account.EMAIL_ACCOUNT;
                txtPURCHASE_1.Text = account.PURCHASE_1;

                dataImport_Auth();
            }
            else
                FCommon.AlertShow(PnMessage_Account, "danger", "系統訊息", "帳號不存在");
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (FCommon.getAuth(this))
            {
                ...
                if (!IsPostBack)
                {   //頁面第一次加載時要執行的事件，當不是第一次加載時不執行
                    FCommon.Controls_Attributes("readonly", "true", txtUSER_ID, txtOVC_DEPT_CDE, txtOVC_ONNAME);
                    #region 下拉式選單匯入資料
                    //系統別
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (strMessage.Equals(string.Empty))
            {
                ...
                GME.SaveChanges();
                FCommon.syslog_add(Session["userid"].ToString(), Request.ServerVariables["REMOTE_ADDR"].ToString(), account.GetType().Name.ToString(), this, "修改");
        ...
        protected void btnAuth_Click(object sender, EventArgs e)
        {
            ...
            if (strMessage.Equals(string.Empty))
            {
                ...
                GME.SaveChanges();
                FCommon.syslog_add(Session["userid"].ToString(), Request.ServerVariables["REMOTE_ADDR"].ToString(), accountAuth.GetType().Name.ToString(), this, "新增");
                dataImport_Auth();
        ...
        {
            FCommon.Controls_Clear(txtUSER_NAME, txtIUSER_PHONE, txtOVC_DEPT_CDE, txtOVC_ONNAME, txtPURCHASE_1, txtEMAIL_ACCOUNT, txtPWD, txtPWDConfirm);
        }
