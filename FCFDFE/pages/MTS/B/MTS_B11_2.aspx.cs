...
{
    public partial class MTS_B11_2 : Page
    {
        ...
        private void dataImport()
        {
            ...
            if (bld != null)
            {
                ...
                txtONB_ITEM_VALUE.Text = bld.ONB_ITEM_VALUE.ToString();
                FCommon.list_setValue(drpONB_CARRIAGE_CURRENCY, bld.ONB_CARRIAGE_CURRENCY_I);
                txtONB_INS_AMOUNT.Text = bld.ONB_ITEM_VALUE.ToString();
                FCommon.list_setValue(drpONB_CARRIAGE_CURRENCY2, bld.ONB_CARRIAGE_CURRENCY_I);
        ...
        protected void btnNew_Click(object sender, EventArgs e)
        {
            if (Page.IsValid == true)
            {
                ...
                string strOVC_CHI_NAME = lblOVC_CHI_NAME.Text;
                string strONB_QUANITY = lblONB_QUANITY.Text;
...
#region 取得投保通知書編號
                ...
                if (strMessage.Equals(string.Empty))
                {
                    try
                    {
                        ...
                        MTSE.SaveChanges();
                        FCommon.syslog_add(Session["userid"].ToString(), Request.ServerVariables["REMOTE_ADDR"].ToString(), iinn.GetType().Name.ToString(), this, "新增");
        ...
        protected void btnNoNeed_Click(object sender, EventArgs e)
        {
            TBGMT_BLD bld = MTSE.TBGMT_BLD.Where(table => table.OVC_BLD_NO.Equals(id)).FirstOrDefault();
            if (bld != null)
            {
                string strUserId = Session["userid"].ToString();
                bld.ODT_MODIFY_DATE = DateTime.Now;
                ...
                FCommon.AlertShow(PnMessage, "success", "系統訊息", $"提單編號：{ id } 設定完成 毋須投保。");
            }
            else
                FCommon.AlertShow(PnMessage, "danger", "系統訊息", $"提單編號：{ id } 不存在！");
        }
        #endregion
