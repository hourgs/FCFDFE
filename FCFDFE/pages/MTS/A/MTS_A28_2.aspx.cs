...
{
    public partial class MTS_A28_2 : Page
    {
        ...
        private void dataImport()
        {
            ...
            if (ECL != null)
            {
                txtOVC_CLASS_CDE.Text = ECL.OVC_CLASS_CDE;
                txtOVC_CLASS_NAME.Text = ECL.OVC_CLASS_NAME;
                txtOVC_ECL_NO.Text = ECL.OVC_ECL_NO;
                txtOVC_EXP_TYPE.Text = ECL.OVC_EXP_TYPE;
                txtOVC_SHIP_CDE.Text = ECL.OVC_SHIP_CDE;
                txtOVC_PACK_NO.Text = ECL.OVC_PACK_NO;
                txtODT_EXP_DATE.Text = FCommon.getDateTime(ECL.ODT_EXP_DATE);
                txtOVC_STORED_PLACE.Text = ECL.OVC_STORED_PLACE;
            }
            ...
            if (strMessage.Equals(string.Empty))
            {
                try
                {
                    ...
                    if (ecl != null)
                    {
                        ...
                        MTSE.SaveChanges();
                        FCommon.syslog_add(strUserId, Request.ServerVariables["REMOTE_ADDR"].ToString(), ecl.GetType().Name, this, "修改");

                        FCommon.AlertShow(PnUpdate, "success", "系統訊息", $"提單編號：{ id } 之出口報單 修改成功。");
                    }
                    else
                        FCommon.AlertShow(PnUpdate, "danger", "系統訊息", $"提單編號：{ id } 之出口報單 不存在！");
                }
                catch
                {
