...
{
    public partial class MTS_A13_2 : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (FCommon.getAuth(this))
            {
                ...
                if (FCommon.getQueryString(this, "id", out id, true))
                {
                    if (!IsPostBack)
                    {
                        FCommon.Controls_Attributes("readonly", "true", txtOVC_DEPT_CDE, txtOVC_ONNAME, txtODT_RECEIVE_INF_DATE, txtODT_IMPORT_DATE,
                            txtODT_PASS_CUSTOM_DATE, txtODT_ABROAD_CUSTOM_DATE, txtODT_UNPACKING_DATE, txtODT_CHANGE_BLD_DATE,
                            txtODT_TRANSMTSR_DATE, txtODT_CUSTOM_DATE, txtODT_RECEIVE_DATE);
                        #region 匯入下拉式選單
                        ...
                        TBGMT_BLD codetable = MTSE.TBGMT_BLD.Where(table => table.OVC_BLD_NO.Equals(id)).FirstOrDefault();
                        if (codetable != null)
                        {
                            ...
                            FCommon.list_setValue(drpOVC_IS_SECURITY, (codetable.OVC_IS_SECURITY ?? 0).ToString());
                        }
                    }
                }
            }  
        ...
        {
            Response.Redirect($"MTS_A13_1{ getQueryString() }");
        }
        ...
        protected void btnNew_Click(object sender, EventArgs e)
        {
            ...
            string strODT_TRANSMTSR_DATE = txtODT_TRANSMTSR_DATE.Text;
            ...
            DateTime dateODT_RECEIVE_INF_DATE, dateODT_IMPORT_DATE, dateODT_PASS_CUSTOM_DATE, dateODT_ABROAD_CUSTOM_DATE, dateODT_UNPACKING_DATE,
                dateODT_CHANGE_BLD_DATE, dateODT_TRANSMTSR_DATE, dateODT_CUSTOM_DATE, dateODT_RECEIVE_DATE, dateNow = DateTime.Now;
            ...
            if (ICR != null)strMessage += $"<P> 提單編號：{ id } 之時程管制簿 已存在！ </p>";
            ...
            bool boolODT_IMPORT_DATE = FCommon.checkDateTime(strODT_IMPORT_DATE, "進口日期", ref strMessage, out dateODT_IMPORT_DATE);
            ...
            bool boolODT_TRANSMTSR_DATE = FCommon.checkDateTime(strODT_TRANSMTSR_DATE, "清運日期", ref strMessage, out dateODT_TRANSMTSR_DATE);
            ...
            TBGMT_IDR IDR = MTSE.TBGMT_IDR.Where(table => table.OVC_BLD_NO.Equals(id) && DateTime.Compare(table.ODT_IDR_DATE, dateODT_RECEIVE_DATE) == 0).FirstOrDefault();
            if (IDR != null) strMessage += $"<P> 提單編號：{ id } 之日報表 已存在！ </p>";
            #endregion
            ...
            if (strMessage.Equals(string.Empty))
            {
                try
                {
                    ...
                    BLD_LOG.OVC_MODIFY_LOGIN_ID = strUserId;
                    ...
                    if (boolODT_IMPORT_DATE) ICR.ODT_IMPORT_DATE = dateODT_IMPORT_DATE; else ICR.ODT_IMPORT_DATE = null;
                    ...
                    if (boolODT_TRANSMTSR_DATE) ICR.ODT_TRANSFER_DATE = dateODT_TRANSMTSR_DATE; else ICR.ODT_TRANSFER_DATE = null;
                    ...
                    ICR.OVC_CREATE_LOGIN_ID = strUserId;
                    MTSE.TBGMT_ICR.Add(ICR);
                    FCommon.syslog_add(strUserId, Request.ServerVariables["REMOTE_ADDR"].ToString(), ICR.GetType().Name.ToString(), this, "新增");
                    #endregion

                    #region TBGMT_ICR_MRPLOG 新增LOG
                    TBGMT_ICR_MRPLOG ICR_LOG = new TBGMT_ICR_MRPLOG();
                    ICR_LOG.LOG_LOGIN_ID = strUserId;
                    ICR_LOG.LOG_EVENT = "INSERT";
                    ICR_LOG.LOG_TIME = dateNow;
                    ICR_LOG.OVC_BLD_NO = id;
                    ICR_LOG.OVC_PURCH_NO = strOVC_PURCH_NO;
                    ICR_LOG.OVC_CHI_NAME = strOVC_CHI_NAME;
                    ...
                    if (boolODT_IMPORT_DATE) ICR_LOG.ODT_IMPORT_DATE = dateODT_IMPORT_DATE; else ICR_LOG.ODT_IMPORT_DATE = null;
                    ...
                    if (boolODT_TRANSMTSR_DATE) ICR_LOG.ODT_TRANSFER_DATE = dateODT_TRANSMTSR_DATE; else ICR_LOG.ODT_TRANSFER_DATE = null;
                    ...
                    ICR_LOG.OVC_CREATE_LOGIN_ID = strUserId;
                    ...
                    #region TBGMT_IDR 新增資料
                    IDR = new TBGMT_IDR();
                    IDR.OVC_BLD_NO = id;
                    if (boolODT_RECEIVE_DATE)
                        IDR.ODT_IDR_DATE = dateODT_RECEIVE_DATE;
                    IDR.ODT_MODIFY_DATE = dateNow;
                    IDR.OVC_CREATE_LOGIN_ID = strUserId;
                    MTSE.TBGMT_IDR.Add(IDR);
                    MTSE.SaveChanges();
                    FCommon.syslog_add(strUserId, Request.ServerVariables["REMOTE_ADDR"].ToString(), ICR_LOG.GetType().Name.ToString(), this, "新增");
                    #endregion

                    FCommon.AlertShow(Pnwarning, "success", "系統訊息", $"提單編號：{ id } 之時程管制簿 及 日報表 建立成功！");
                }
                catch
                {
                    FCommon.AlertShow(Pnwarning, "danger", "系統訊息", $"提單編號：{ id } 建立失敗，請聯絡工程師！");
                }
            }
            else
