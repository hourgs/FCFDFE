...
{
    public partial class MTS_A18_2 : Page
    {
        ...
        private void dataImport(string strOVC_BLD_NO) //載入本頁基本資料
        {
            ...
            if (irdTable != null)
            {
                ...
                txtODT_CLEAR_DATE.Text = FCommon.getDateTime(irdTable.ODT_CLEAR_DATE); //清檢日期 => 提單/拆櫃日期

                txtONB_ACTUAL_RECEIVE.Text = irdTable.ONB_ACTUAL_RECEIVE.ToString(); //實收
                txtONB_OVERFLOW.Text = irdTable.ONB_OVERFLOW.ToString(); //溢卸
                txtONB_LESS.Text = irdTable.ONB_LESS.ToString(); //短少
                txtONB_BROKEN.Text = irdTable.ONB_BROKEN.ToString(); //破損
                txtOVC_NOTE.Text = irdTable.OVC_NOTE; //備考
            }
        }
        ...
        private void dataImport_DETAIL(bool isCreate) //載入分運明細資料表GV
        {
            ...
            if (!strOVC_BLD_NO.Equals(string.Empty))
            {
                ...
                if (IsPostBack) //第一次進入帶資料庫資料，不須做運算
                {
                    //計算清驗情形
        ...
        protected void btnModify_Click(object sender, EventArgs e)
        {
            ...
            if (!strOVC_BLD_NO.Equals(string.Empty))
            {
                ...
                TBGMT_IRD irdTable = MTSE.TBGMT_IRD.Where(table => table.OVC_BLD_NO.Equals(strOVC_BLD_NO)).FirstOrDefault();
                if (irdTable == null) strMessage += "<p> 請先新增 接配紀錄表！ </p>";
                #region 檢查欄位輸入
                ...
                if (strMessage.Equals(string.Empty))
                {
                    ...
                    #region 修改 IRD
                    //不須修改 irdTable.ODT_ARRIVE_PORT_DATE = dateODT_ARRIVE_PORT_DATE; //到港日期 => 進口日期
                    ...
                    MTSE.SaveChanges(); //儲存
                    FCommon.syslog_add(Session["userid"].ToString(), Request.ServerVariables["REMOTE_ADDR"].ToString(), irdTable.GetType().Name.ToString(), this, "修改");
                    #endregion

                    //FCommon.AlertShow(PnMessage, "success", "系統訊息", "修改接配紀錄表成功");
                    FCommon.AlertShow(PnMessage, "success", "系統訊息", $"提單編號：{ strOVC_BLD_NO } 修改接配紀錄表成功");
                }
                else
        ...
        protected void GVTBGMT_CTN_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                ...
                case "DataSave": //儲存
                    ...
                    break;
                case "DataDelete": //刪除
                    //Label lblOVC_CONTAINER_NO = (Label)gvr.FindControl("lblOVC_CONTAINER_NO");
                    if (!strOVC_BLD_NO.Equals(string.Empty))
                    {
                        TBGMT_IRD_CTN IrdCtnTable = MTSE.TBGMT_IRD_CTN
