...
{
    public partial class MPMS_C16_1 : System.Web.UI.Page
    {
        bool hasRows = false;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (FCommon.getAuth(this))
            {
                ...
                if (string.IsNullOrEmpty(Request.QueryString["Auditor"]) ||
                    string.IsNullOrEmpty(Request.QueryString["YearSub"]))
                {
                    Response.Redirect("MPMS_C25");
                }
                else
                {
                    auditor = Request.QueryString["Auditor"];
                    yearSub = Request.QueryString["YearSub"];
                    if (!IsPostBack)
                    {
                        GetUserInfo();
                        GVDataImport(yearSub);
                    }
                }
            }
        ...
        protected void GV_NOT_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                Label lblOVC_AUDITOR = (Label)e.Row.FindControl("lblOVC_AUDITOR");
                ...
                if (string.IsNullOrEmpty(hidOVC_DRESULT.Value))
                {
                    ...
                    {
                        lblDate.Text = "回覆日<br/>" + hidOVC_DAUDIT.Value;
                        lblDate.Visible = true;
                        drp.Visible = false;
                        lblOVC_AUDITOR.Visible = true;
                    }
                }
                else
                {
        ...
        private void GVDataImport(string year)
        {
            ...
            var query =
                ...
                where t1202_1.OVC_PURCH.Substring(2, 2).Equals(year)
                    && t1202_1.OVC_AUDITOR.Equals(auditor)
            ...
            if (Session["userid"] != null)
            {
                ...
                var query_Auth =
                     (from tAccount in mpms.ACCOUNT
                      ...
                      where tAuth.C_SN_AUTH.Equals("3502")
                      select new
                      {
                          tAccount.DEPT_SN,
                          tAuth.C_SN_SUB
        ...
        {
            Response.Redirect("MPMS_C25.aspx");
        }
        #endregion
